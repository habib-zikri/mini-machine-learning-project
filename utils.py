...
def generate_student_dataset(file_path):
    ...
    """
    # Mengatur seed agar hasil generate dataset selalu konsisten
    np.random.seed(42)
    
    n_samples = 100
    
    # 1. Generate Nama Mahasiswa
    nama_list = [f"Mahasiswa_{i+1:03d}" for i in range(n_samples)]
    
    # 2. Generate Jam Belajar per Minggu (antara 2 hingga 20 jam)
    jam_belajar = np.random.uniform(2.0, 20.0, n_samples)
    
    # 3. Generate Status Mengikuti Bimbel (Ya / Tidak)
    ikutan_bimbel = np.random.choice(["Ya", "Tidak"], size=n_samples, p=[0.4, 0.6])
    
    # 4. Hitung Nilai Ujian berdasarkan Jam Belajar, Bimbel, dan sedikit noise acak
    # Rumus dasar: Nilai = 45 + (Jam_Belajar * 2.2) + (Bimbel_Bonus) + Noise
    ...
    # Memastikan nilai berada di rentang 0-100 dan membulatkannya
...
def fit_linear_regression(x, y):
    ...
    # Menghitung pembilang (numerator) dan penyebut (denominator)
    numerator = np.sum((x - mean_x) * (y - mean_y))
    denominator = np.sum((x - mean_x) ** 2)
    
    # Slope (m) dan Intercept (c)
